...
- `/formae:setup` now works on a machine where nothing is configured. It finds out what the machine holds, asks hosted or self-hosted, signs in or creates a profile, confirms what was written, and only then checks the agent — so a machine with no agent yet no longer reports a setup failure that is not about setup.
