...
Explain changes as keep/revert decisions in either source mode, never infer acceptance from a request to add or edit another property. Read the current drift preference; auto_absorb permits proposing absorb for external changes and formae patches, including mixed history and deletions, without a new keep/revert choice. Legacy auto_absorb_external only permits this for server-proven ExternalChangesOnly=true. The agent merge must still check conflicts; conflicts always require a decision. Resolve actionable drift with all absorb/revert choices keyed by ResourceID: initial soft simulation returns ObservationID; simulate resolution Decisions to obtain the final combined ReviewID; after confirmation submit it with a stable IdempotencyKey. Retain exact input and key for uncertain-outcome retry. stale-review needs a fresh observation, plan and confirmation, never silent force. Pure acceptance is finished centrally by a real command; source catch-up is a separate harness edit using the partial get_command_desired_delta guidance for only the selected project.
...
const PrepareBugReportDescription = `Prepare an immutable sanitized bug report for hosted formae support, from any workflow. Nothing is submitted. Prefer event_id from the original failed call; only use an explicit hosted profile for an issue outside captured calls and disclose that provenance. Reads the configured support recipient from the authenticated hosted console (default support@platform.engineering), then returns that recipient with the complete preview and report_id. The report and diagnostics are not uploaded during preparation. Include only relevant local CLI diagnostics; hosted agent logs are already available to support. Never include inventories, full forma files, transcripts or arbitrary log files. Available for this MCP session for one hour. Classic users should use GitHub issues or Discord.`
const SubmitBugReportDescription = `Send a previously prepared bug report to its displayed support recipient only with user authorization and confirmed=true. Accepts only report_id and confirmed; submits exactly the sanitized preview to its original hosted installation. Existing explicit session authorization may cover this send. No infrastructure operation is retried. Retry delivery only with the same report_id. A delivery_unknown receipt does not mean email was sent. If the configured recipient changes, prepare a new preview and obtain authorization for its destination. Prepared reports expire after one hour and are isolated to this MCP session.`
