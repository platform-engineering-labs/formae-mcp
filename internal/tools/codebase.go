...
type DriftPreferenceInput struct {
	...
	Mode    string `json:"mode" jsonschema:"required,prompt or auto_absorb. Set only after explicit consent. auto_absorb covers nonconflicting external changes and formae patches including deletions; conflicts still require a decision. Legacy auto_absorb_external remains supported with external-only scope."`
}

const DriftPreferenceDescription = `Save the user's local drift preference for the resolved installation, across sessions and codebases. Only call after an explicit user choice. While the preference is unset and readable, offer after an explicit keep: ask each time (prompt), or automatically keep future nonconflicting external changes and formae patches, including deletions (auto_absorb). Ask once during the operation; do not offer after revert alone. Keeping the current change is not consent to a future policy. A saved explicit prompt suppresses future preference offers. No answer leaves the default unchanged and does not block the current apply. Show automatic acceptance and deletions leaving desired state in the final preview and message. The preference never authorizes force, overriding conflicts, or skipping ordinary cloud-write confirmation. Existing auto_absorb_external consent remains external-only; never silently broaden it. Read the effective preference through get_codebase_context; apply rejections refresh it. The user can change it at any time.`
...
const RegisterCodebaseDescription = `Register an existing local IaC project for the resolved installation after the user opts into maintaining it. The directory must already contain PklProject. This records the canonical project path and optional stack scope; initialization and codebase catch-up are separate steps. When the user is opting in from no-code mode, ask where the persistent project should live first, then follow the existing formae-project-init workflow and formae://docs/forma-structure guidance to create the conventional layout (root entry point, shared vars/targets, and per-stack files) before calling this tool. Do not register the disposable prepare_authoring directory or invent a second project layout. Multiple projects can track one installation.`
