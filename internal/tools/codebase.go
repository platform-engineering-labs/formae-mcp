...
const DriftPreferenceDescription = `Save the user's local drift preference for the resolved installation, across sessions and codebases. Only call after an explicit user choice. Offer on the first external change alongside the current keep/revert decision: ask each time (prompt), or automatically keep future nonconflicting external changes, including external deletions (auto_absorb_external). Show accepted deletions as removals from desired state in the final preview and message. Patches always require a decision. The preference never authorizes force, conflicting changes, guessing unknown provenance, or skipping ordinary cloud-write confirmation. Read the effective preference through get_codebase_context; apply rejections refresh it. The user can change it at any time.`
