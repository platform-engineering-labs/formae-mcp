...
type ListCommandsInput struct {
	Query      string `json:"query,omitempty" jsonschema:"Query to filter commands. Supported fields: id, client, command (apply/destroy), status (pending/in_progress/completed/failed), stack, managed. Use 'client:me' to filter to your own commands. Leave empty for the agent's recent command history."`
...
type ApplyFormaInput struct {
	Context    *SourceContext   `json:"context,omitempty" jsonschema:"Carry the selected source context on every simulation and real apply, including patch: get_codebase_context for a maintained binding or prepare_authoring context for disposable source. Optional only for legacy callers. A maintained file and every evaluated stack must stay inside the binding scope."`
	Resolution *DriftResolution `json:"resolution,omitempty" jsonschema:"Explicit shared drift decisions. Simulate all choices, review the combined plan, then confirm and submit its ReviewID with a stable IdempotencyKey. On stale-review obtain a fresh observation and confirmation; never force."`
	Message    *string          `json:"message,omitempty" jsonschema:"Optional command intent message recorded in history. Suggest it with the plan; the user may accept, edit or clear it. Never include secrets or claim success before execution."`
	...
	Mode       string           `json:"mode" jsonschema:"required,Apply mode. Use 'reconcile' without force for ordinary changes, even one resource or label, with the complete affected stack declaration. Omitted resources within that stack are destroyed. Use 'patch' only for an explicit patch request or stated incident/hotfix intent requiring temporary intervention. Small scope, quick wording or existing drift is not a reason to patch; never bypass drift decisions by changing mode."`
...
type DestroyFormaInput struct {
	Context  *SourceContext `json:"context,omitempty" jsonschema:"Per-call source context for file-based destruction. Query destruction has no local source context."`
...
type CreateInlinePolicyInput struct {
	Profile         string         `json:"profile,omitempty" jsonschema:"Installation profile for this policy planning call."`
	Context         *SourceContext `json:"context,omitempty" jsonschema:"Selected codebase or disposable source context. Requires explicit forma_file; scans stay inside this selected root."`
...
type DeleteStandalonePolicyInput struct {
	FormaFile string         `json:"forma_file,omitempty" jsonschema:"Explicit policy declaration file; required with context."`
	Profile   string         `json:"profile,omitempty" jsonschema:"Installation profile for this policy planning call."`
	Context   *SourceContext `json:"context,omitempty" jsonschema:"Selected codebase or disposable source context. Requires explicit forma_file; scans stay inside this selected root."`
...
// DriftResolution matches the connected agent's recorded review contract.
type DriftResolution struct {
	ObservationID  string          `json:"ObservationID" jsonschema:"required,ObservationID from the initial soft reconcile rejection."`
	Decisions      []DriftDecision `json:"Decisions" jsonschema:"required,Exactly one absorb or revert decision per actionable ResourceID; no skip."`
	ReviewID       string          `json:"ReviewID,omitempty" jsonschema:"Final composed simulation ReviewID, required for real submission."`
	IdempotencyKey string          `json:"IdempotencyKey,omitempty" jsonschema:"Stable caller-generated key for this exact real submission. Retain unchanged through retries until its outcome is known."`
}
type DriftDecision struct {
	ResourceID string `json:"ResourceID" jsonschema:"required,Stable resource identity from the observation."`
	Action     string `json:"Action" jsonschema:"required,absorb or revert."`
}

// SourceContext is local only; it is never sent to the agent.
type SourceContext struct {
	Mode               string `json:"mode" jsonschema:"required,codebase or none. No global active source context exists."`
	BindingID          string `json:"binding_id,omitempty" jsonschema:"Selected registered project binding for codebase mode."`
	TemporaryDirectory string `json:"temporary_directory,omitempty" jsonschema:"Caller-owned disposable directory returned by prepare_authoring for none mode. Retain through review/submission and delete after outcome is known."`
}

type CommandDesiredDeltaInput struct {
	CommandID string `json:"command_id" jsonschema:"required,Terminal command whose recorded desired contributions should guide source catch-up."`
	Profile   string `json:"profile,omitempty" jsonschema:"Installation profile for this call."`
}
