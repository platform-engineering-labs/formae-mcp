...
type ListCommandsInput struct {
	Query      string `json:"query,omitempty" jsonschema:"Query to filter commands. Supported fields: id, client, user, command (apply/destroy), status (pending/in_progress/completed/failed), stack, managed. Use 'client:me' for this session's own commands (this machine's formae client id, shared with the CLI when its id file exists, else the shared 'formae-mcp' fallback; not the human) or 'user:me' for the human behind the bearer token; a user UUID matches the subject id, anything else matches the display name. Leave empty for most recent commands."`
