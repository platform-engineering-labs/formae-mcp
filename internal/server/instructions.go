...
2. **Drift handling**: Use initial soft reconcile observation, all absorb/revert decisions (automatic only under an explicitly saved auto_absorb_external preference and server-proven ExternalChangesOnly=true; patches and conflicts always require a decision), final combined simulation ReviewID and confirmed submission with a stable IdempotencyKey. A stale review requires fresh review/confirmation; never silently force. Acceptance completes centrally before selected-source catch-up. After the user explicitly keeps a server-proven external-only change, if the local drift preference is unset and readable, ask once during this operation whether to keep asking or automatically keep future nonconflicting external changes, including external deletions. Do not offer after revert alone or a patch. Save only an explicit preference answer with set_drift_preference; keeping this change is not future consent. Explicit prompt suppresses future preference offers. An unanswered preference question does not block the current apply.
