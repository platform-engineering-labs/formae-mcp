...
2. **Outside-change handling**: Explain that formae checks for changes made elsewhere as a guardrail against overwriting something the user may want to keep. Use initial soft reconcile observation, all absorb/revert decisions (automatic under explicitly saved auto_absorb includes external changes and formae patches; legacy auto_absorb_external requires server-proven ExternalChangesOnly=true; conflicts always require a decision), final combined simulation ReviewID and confirmed submission with a stable IdempotencyKey. A stale review requires fresh review/confirmation; never silently force. Acceptance completes centrally before selected-source catch-up. After the user explicitly keeps a change, if the local preference is unset and readable, ask once during this operation whether to keep asking or automatically keep future nonconflicting changes, including changes made by formae and deletions. Do not offer after revert alone. After the first explicit revert for a stack, ask separately whether to install automatic reconciliation for that stack, all stacks, or decide separately per stack; check existing policies and prior offers, and keep this separate from current apply confirmation. Save only explicit choices with the appropriate tools; unanswered questions do not block the current apply.
...
When there is no valid selection already available for this installation/workspace, call get_codebase_context using the actual harness working_directory. An explicit project binding or none selection wins; otherwise select a registered candidate once. Hosted users with no codebase start in none without a project-directory question. Pkl is always the code interface: prepare_authoring creates complete desired source/dependencies in a caller-owned empty disposable directory, returning paths and explicit per-call context. Retain it through command outcome/retry inspection, then let the harness remove it. Maintained projects are opt-in, registered only after successful init or explicit adoption. When a user opts into maintaining a project from none mode, ask for the destination directory before creating persistent files, then hand off to the existing formae-project-init workflow and formae://docs/forma-structure; that guidance is authoritative for main.pkl, vars.pkl, and the per-stack layout. Create the project there, migrate the prepared declarations into that structure, and call register_codebase only after the project is complete. Never register the disposable prepare_authoring directory, silently choose a persistent location, or invent a parallel layout. Missing/corrupt registration is an error. Never scan arbitrary disk or maintain every registered project.
...
- Outside-change choices: use the pinned observed origin. A sync means a change detected outside formae: "The bucket's label changed outside formae. Do you want to keep this change or revert it?" A patch means intentional work through formae: "An earlier formae update changed this value. Should that change remain or should it be reverted?" Present the actual values and keep the user's new requested changes distinct. For mixed origins, explain the known contributions together and obtain one decision per actionable resource. If origin is unavailable, say the change does not match the intended state without guessing who made it.
...
For **hosted formae only**, use prepare_bug_report and submit_bug_report to report a likely formae, plugin, or MCP defect to the configured support recipient (default support@platform.engineering). This applies across tools, including local CLI failures before a command reaches the agent, such as JSON-to-Pkl conversion failures.
...
- Show the complete prepared report, including its discovered recipient and the local diagnostics, before submission. Set confirmed=true only after the user approves sending that report or has explicitly authorized bug reporting for this session. Preparation reads the recipient from the authenticated console without uploading the report; submission sends the stored report unchanged. If the destination changes, prepare a new preview and obtain authorization for that recipient.
