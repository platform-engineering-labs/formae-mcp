...
2. **Drift handling**: Use initial soft reconcile observation, all absorb/revert decisions (automatic under explicitly saved auto_absorb includes external changes and formae patches; legacy auto_absorb_external requires server-proven ExternalChangesOnly=true; conflicts always require a decision), final combined simulation ReviewID and confirmed submission with a stable IdempotencyKey. A stale review requires fresh review/confirmation; never silently force. Acceptance completes centrally before selected-source catch-up. After the user explicitly keeps a change, if the local drift preference is unset and readable, ask once during this operation whether to keep asking or automatically keep future nonconflicting external changes and formae patches, including deletions. Do not offer after revert alone. Save only an explicit preference answer with set_drift_preference; keeping this change is not future consent. Explicit prompt suppresses future preference offers. An unanswered preference question does not block the current apply.
