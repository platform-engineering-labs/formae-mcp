...
# Each case runs in a subshell with its own HOME and PATH, and an install_pkg
# stub that records what it was asked to install instead of downloading anything.
run_case() {
    _name="$1"
    _setup="$2"
    _want_managed="$3"
    _want_installs_expr="$4"
    _want_bin_expr="$5"
    ...
    (
        ...
        # A pelmgr that fails is on PATH so that nothing in a test can reach the
        # hub, whichever path a regression takes to it.
        printf '#!/bin/sh\nexit 1\n' > "$fakepath/pelmgr"; chmod +x "$fakepath/pelmgr"
        installs=""

        # install_pkg is the one seam through which anything gets downloaded.
        # The stub records each call as <tree>:<pkg> and lays down what the
        # real install would, so the fast paths above it run for real.
        # shellcheck disable=SC2317
        install_pkg() {
            installs="$installs${installs:+ }$1:$3"
            case "$3" in
                formae) stub_bin "$1/bin/formae" ;;
                oidc)   mkdir -p "$1/formae/plugins/oidc/v0.1.0" ;;
            esac
            return 0
        }
        ...
        expect "$_name: installs" "$installs" "$(eval echo "$_want_installs_expr")"
...
run_case "own install on PATH" \
    ...
    0 '$HOME/.pel:oidc' '$fakepath/formae'
...
run_case "user-local install off PATH is found" \
    ...
    0 '$HOME/.pel:oidc' '$HOME/.local/bin/formae'
...
run_case "a symlinked own install resolves to its target" \
    ...
    0 '$HOME/.pel:oidc' '$HOME/real/formae'

# A user-owned formae cannot be completed in place: its plugin directory sits
# beside a binary we did not install, root-owned for a system install. formae
# also searches ~/.pel/formae/plugins, which is the user's to write, so the
# auth plugin a hosted sign-in needs is installed there instead.
run_case "own install gets the auth plugin in the user plugin dir" \
    'stub_bin "$fakepath/formae"' \
    0 '$HOME/.pel:oidc' '$fakepath/formae'

# Already there means nothing to download: the plugin has no binary under bin/
# for the package fast-path to see, so the check is on the plugin directory.
run_case "own install with the auth plugin present installs nothing" \
    'stub_bin "$fakepath/formae"; mkdir -p "$HOME/.pel/formae/plugins/oidc/v0.1.0"' \
    0 '' '$fakepath/formae'

# The same fast-path serves the managed tree, where the plugin used to be
# reinstalled on every launch for want of a binary to check.
run_case "managed copy with the auth plugin present installs nothing" \
    'stub_bin "$managed"; mkdir -p "$HOME/.formae-ai/opt/formae/plugins/oidc/v0.1.0"' \
    1 '' '$managed'
